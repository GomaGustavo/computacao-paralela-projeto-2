...
__global__ void cuda_reduce(double* input, double *output, int width) {
  ...
  // declaração de um vetor na memória local do tamanho do bloco
  __shared__ double aux[1024];  

  // buscando o dado da memória global
  if(b+t < width)
    aux[t] = input[b+t];

  // sincronizando as threads de um mesmo bloco
  __syncthreads();
  
  int i;

  // a cada iteração, metade das threads continuam proceessando
  for(i = blockDim.x / 2; i > 0; i /= 2) {
    if(t < i && b + t + i < width) // verifica se existe posição no vetor
      aux[t] += aux[t+i];
